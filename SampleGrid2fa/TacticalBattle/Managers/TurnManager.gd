...
func _perform_ai_movement(unit: Unit) -> void:
	...
	if game_board and game_board._grid_state_manager:
		# Make sure we're within grid bounds
		if unit.grid.is_within_bounds(new_cell) and not game_board._grid_state_manager.is_occupied(new_cell):
			# Update unit position via GridStateManager
			game_board._grid_state_manager.move_unit(unit, unit.cell, new_cell)
